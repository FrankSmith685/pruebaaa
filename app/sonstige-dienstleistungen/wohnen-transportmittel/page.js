...
const WohnenTransport = () => {
    ...
    const [images, setImages] = useState([imagenRef.current.Wohen, imagenRef.current.Transportmittel]);
    const [loading, setLoading] = useState(true); 

    useEffect(() => {
    const checkImage = () => {
        if (imagenRef?.current?.Wohen && imagenRef.current.Transportmittel) {
            setImages([imagenRef.current.Wohen,imagenRef.current.Transportmittel]);
            setLoading(false);
        } else {
